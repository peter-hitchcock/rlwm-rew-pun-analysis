RunHWMPRew <- function(parameters, block, helpers, prior_Q_RL_values=NULL, eta=.5) {
  ### Run through modified RLWM model ### a
  ...
  init_ck_value <- 1/3 # 1/3 to match Collins 24
  # Set goal-oriented WM values that allow distinguishing nothing in WM from 
  # neutral 
  init_wm_value <- 1/3
  
  CK_mat <- matrix(init_ck_value, n_s, 3)
  ...
  for (tib in 1:nrow(learn_block)) {
    ...
    # CHOICE: Get the action values from CK and WM this trial.. # 
    ck_values <- CK_mat[stim, ]
    wm_values <- Q_mat_wm[stim, ]
    
    # .. pass each through softmax .. (beta hard coded at high value)  
    ck_liks <- unlist(RunLSESoftmax(ck_values))#, debug_info)
    wm_liks <- unlist(RunLSESoftmax(wm_values))#, debug_info)
    
    # cat("\n\n**Trial**", tib, "\n\n")
    # cat("\nStim", stim)
    # cat("\n\nChoice kernel matrix\n:"); print(CK_mat); cat("\n\n")
    # cat("\n\nWM matrix matrix\n:"); print(Q_mat_wm); cat("\n\n")
    # cat("\nChoice kernel values:", ck_values)
    # cat("\nWorking memory values:", wm_values)
    # cat("\nChoice kernel likelihood:", round(ck_liks, 4))
    # cat("\nWM likelihood:", round(wm_liks, 4))
    ...
    if (these_stim_iters[tib] == 6) {
      directed_choice_probs <- ck_liks 
    } else {
      directed_choice_probs <- W_wm*wm_liks + (1-W_wm)*ck_liks
    }
    
    # cat("\nWM proportion contr. directed choice:", W_wm)
    # cat("\n\nDirected choice probs:", directed_choice_probs)
    ...
    alpha_wm_t <- NULL
    alpha_wm_t <- if_else(outcome==1, alpha_wm, learning_bias)
    # cat("\nOutcome", outcome, "\nBias", learning_bias, "\nalpha_wm_t", alpha_wm_t)
    # cat("\nWM before update"); print(Q_mat_wm[stim, action])
    
    Q_mat_wm[stim, action] <- Q_mat_wm[stim, action] + alpha_wm_t * (outcome - Q_mat_wm[stim, action])
    #cat("\nWM after update"); print(Q_mat_wm[stim, action])
    ...
    alpha_ck_t <- NULL
    alpha_ck_scaled_down <- NULL 
    
    
    # Note: learning bias is set up in non-intuitive way so that lower values = more bias
    alpha_ck_t <- if_else(outcome==1, alpha_ck, learning_bias*alpha_ck)
    #cat("\nalpha_ck_t", alpha_ck_t)
    
    # cat("\nAction", action)
    # cat("\nCK learning rate — unscaled", alpha_ck)
    # cat("\nLearning bias", learning_bias)
    # cat("\nOutcome", outcome)
    # cat("\nCK learning rate — after learning bias scaling", alpha_ck_t)
    # cat("\n\nCK matrix before update"); print(CK_mat)
    # cat("\nCK(s,a) before update"); print(CK_mat[stim, action])
    
    #browser(expr=outcome==0 & tib > 11)
    ck_off_t <- NULL
    # If delay is NA then it's the first trial of phase with that stim iter, so learn by standard CK  
    if (is.na(delay_t)) {
      
      ck_out <- LearnByCK(stim, action, CK_mat, alpha_ck)
    # Otherwise use CK off  inv. proportional to delay  
    } else { 
      
      
      ck_off_t <- if_else(delay_t == 0, ck_off, ck_off/delay_t)
      
      alpha_ck_scaled_down <- alpha_ck_t * (1-ck_off_t)
      
      ck_out <- LearnByCK(stim, action, CK_mat, alpha_ck=alpha_ck_scaled_down) 
    ...
    CK_mat <- ck_out[["CK_mat_tplus1"]]
    ck_rpe <- ck_out[["ck_rpe"]]
    
    # cat("\n\nDelay(t)", delay_t)
    # cat("\nCK off - unscaled", ck_off)
    # cat("\nCK off(t) (after delay scaling)", ck_off_t)
    # cat("\nCK learning rate —  after further scaling by CK off(t) ", alpha_ck_scaled_down)
    # cat("\n\nCK(s,a) after update"); print(CK_mat[stim, action])
    # cat("\n\nCK matrix after update"); print(CK_mat)
  ...
  for (testib in 1:nrow(test_block)) { 
    ...
    these_test_ck_vals <- NULL 
    
    if (testib < first_phase_2_test_trial) {
      these_test_ck_vals <- phase_1_CK_mat[test_stim, ]
    } else {
      these_test_ck_vals <- final_CK_mat[test_stim, ]
    }
    
    # Test directed probs just based on CK   
    test_probs <- unlist(RunLSESoftmax(these_test_ck_vals))
