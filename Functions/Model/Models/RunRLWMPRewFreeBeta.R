RunRLWMPRewFreeBeta <- function(parameters, block, helpers, prior_Q_RL_values=NULL, eta=.5) {
  ...
  for (tib in 1:nrow(learn_block)) {
    ...
    rl_liks <- unlist(RunLSESoftmax(rl_values, beta=beta))#, debug_info)
    wm_liks <- unlist(RunLSESoftmax(wm_values, beta=beta))#, debug_info)
