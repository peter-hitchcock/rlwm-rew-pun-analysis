RunRLWMPRewRLCoop <- function(parameters, block, helpers, prior_Q_RL_values=NULL, eta=.5) {
  ...
  for (tib in 1:nrow(learn_block)) {
    ...
    if (is.na(delay_t)) {
      ...
      rl_out <- LearnByRLCooperative(outcome, stim, action, Q_mat_rl, alpha=alpha_t, eta=eta, Q_WM=Q_mat_wm[stim, action])
    } 
